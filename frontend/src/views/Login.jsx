...
function RegisterSheet({ close }) {
  ...
  return <>
    ...
    <div style={{ height: 12 }} />
    <button className="btn primary" onClick={go}>{t('Create passkey')}</button>
  </>
}
...
export default function Login() {
  ...
  return (
    <div className="narrow" style={{ display: 'flex', flexDirection: 'column', justifyContent: 'center', minHeight: '78vh', textAlign: 'center' }}>
      ...
      {webauthnOK() ? <>
        <button className="btn primary" onClick={signIn}>👤 {t('Sign in with passkey')}</button>
        <div style={{ height: 10 }} />
        <button className="btn" onClick={() => useUI.getState().openSheet(close => <RegisterSheet close={close} />)}>✨ {t('Create new profile')}</button>
        <div style={{ height: 10 }} />
      ...
      <button className="btn ghost dim" onClick={() => setGuest(true)}>{t('Continue without account')}</button>
